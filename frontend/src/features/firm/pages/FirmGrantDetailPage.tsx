...
export function FirmGrantDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      if (!Number.isFinite(grantId)) {
        setError('Gecersiz hibe kimligi.')
        return
      }
      ...
      try {
        ...
        if (rule.status === 'fulfilled') {
          ...
          setEligibility(null)
          setEligibilityMessage('Bu cagri icin henuz uygunluk kurali tanimlanmamis.')
        }
      } catch {
        setError('Hibe detayi yuklenemedi.')
      } finally {
        ...
        <>
          <header className="grant-detail-head">
            ...
            <div className="grant-detail-meta-grid">
              ...
              <div className="grant-detail-meta-item">
                <span className="grant-detail-meta-label">Ulke</span>
              ...
              <div className="grant-detail-meta-item">
                <span className="grant-detail-meta-label">Butce Araligi</span>
          ...
          <section className="page-card grant-detail-eligibility">
            <h2>Uygunluk Kurallari</h2>

            {eligibility ? (
              <div className="grant-detail-meta-grid">
                <div className="grant-detail-meta-item">
                  <span className="grant-detail-meta-label">Basvuru Tipleri</span>
                  <strong>{eligibility.applicantTypes?.length ? eligibility.applicantTypes.join(', ') : '-'}</strong>
                </div>
                <div className="grant-detail-meta-item">
                  <span className="grant-detail-meta-label">Sirket Yasi</span>
                  <strong>{formatRange(eligibility.minCompanyAgeMonths, undefined, ' ay')}</strong>
                </div>
                <div className="grant-detail-meta-item">
                  <span className="grant-detail-meta-label">Calisan Araligi</span>
                  <strong>{formatRange(eligibility.minEmployees, eligibility.maxEmployees)}</strong>
                </div>
                <div className="grant-detail-meta-item">
                  <span className="grant-detail-meta-label">Ciro Araligi</span>
                  <strong>{formatRange(eligibility.minTurnover, eligibility.maxTurnover, ' TL')}</strong>
                </div>
                <div className="grant-detail-meta-item">
                  <span className="grant-detail-meta-label">Gerekli Ulkeler</span>
                  <strong>{eligibility.requiredCountryCodes?.length ? eligibility.requiredCountryCodes.join(', ') : '-'}</strong>
                </div>
                <div className="grant-detail-meta-item">
                  <span className="grant-detail-meta-label">Es Finansman</span>
                  <strong>{eligibility.cofundingRequired ? `Gerekli (en az %${eligibility.cofundingRate ?? 0})` : 'Zorunlu degil'}</strong>
                </div>
                {eligibility.notes ? (
                  <div className="grant-detail-meta-item">
          ...
          <div className="inline-actions">
            ...
            <Link to="/app/meetings" className="btn btn-primary">Randevu Panelini Ac</Link>
            <Link to="/app/pre-analysis/history" className="btn">On Analiz Talebi</Link>
          </div>
          <p className="grant-detail-action-help">
            Randevu Panelini Ac: Talep ettiginiz ve onaylanan randevulari takvim gorunumunde takip edersiniz.
          </p>
        </>
      ) : null}
    </section>
